...
def run(name, script_and_args, stale=False):
    ...
    if stale and "validate.py" in script_and_args[0]:
        args.append("--stale")
    result = subprocess.run(args, cwd=str(ROOT.parent))
...
def main():
    ...
    for name, cmd in [
        ...
        ("check_review_signals.py --audit-harnesses", ["check_review_signals.py", "--audit-harnesses"]),
    ]:
        rc = run(name, cmd, stale=args.stale)
        if rc != 0:
