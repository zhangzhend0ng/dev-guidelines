...
- Use `[[Entity Name\|display text]]` when the link text differs from the note title
- Use `[[Entity Name\|context]]` for alias references
...
- If the concept has evolved, add a new line rather than replacing

#### 5.4 Concept Note Discovery

1. Read all `*.md` files in `工作日记/` that are NOT diary files (not matching `工作日记_YYYYMMDD.md`)
2. Each such file represents a concept node in the graph
3. If the new diary mentions a concept with an existing note, link to it
4. If the new diary introduces a concept appearing in 2+ diaries or flagged as significant, create a new note
...
### Step 7: Push to Feishu Wiki

After generating the local diary file, append the report to the designated Feishu wiki document.

#### Configuration

| Setting | Value |
|---------|-------|
| Feishu Wiki URL | `https://snapmaker.feishu.cn/wiki/M0G1wvL7FiZXoSkNGvUc3C8InHh` |
| Wiki Node Token | `M0G1wvL7FiZXoSkNGvUc3C8InHh` |
| Space ID | `7628802645330775239` |
| Docx Object Token | `MEcXdJwzUoyPBKxQlYlcKi7ynnb` |

#### Push Procedure

1. **Load the lark-doc skill** — invoke `Skill` with `lark-doc` to get access to docx read/write tools.

2. **Read the current document** — use `lark-cli docx +fetch --docx-token MEcXdJwzUoyPBKxQlYlcKi7ynnb --format markdown` to get the existing content and understand the current structure.

3. **Format the report for Feishu** — convert the daily report from the local markdown file into a format suitable for appending to the Feishu docx:
   - Use `## 工作日记 YYYY-MM-DD` as the top-level heading for the day's entry
   - Preserve the structured sections (工作内容, 产出物评估, 工作量总结, 总结)
   - Keep tables intact (Feishu docx supports markdown tables)
   - Remove Obsidian-specific `[[]]` wiki-links if present — replace with plain text

4. **Append to document** — use `lark-cli docx +append --docx-token MEcXdJwzUoyPBKxQlYlcKi7ynnb --content "<markdown content>"` to add the day's report at the end of the document.
   - If the document is empty or only has a title, the first entry establishes the structure.
   - If previous entries exist, add a separator (`---`) before the new day's content.
   - Always use `--as user` for docx write operations.

5. **Verify** — after pushing, confirm the operation succeeded by checking the document structure. Report the Feishu document URL to the user.

#### Error Handling

- If `lark-cli` is not authenticated, run `lark-cli auth login --as user` and guide the user through the OAuth flow.
- If scope/permission errors occur, check that the app has `docx:document:write` and `wiki:node:read` scopes enabled in the Feishu developer console.
- If the document token has changed (e.g., after a doc replacement), re-resolve it via `lark-cli wiki spaces get_node --params '{"token":"M0G1wvL7FiZXoSkNGvUc3C8InHh"}' --format json`.
